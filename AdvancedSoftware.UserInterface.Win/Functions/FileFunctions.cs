...
namespace AdvancedSoftware.UserInterface.Win.Functions
{
    public static class FileFunctions
    {
        public static void FormSablonKaydet(this string sablonAdi, int left, int top, int width, int height, FormWindowState windowState)
        {
            try
            {
                if (!Directory.Exists(Application.StartupPath + @"\Sablon Dosyaları"))
                    Directory.CreateDirectory(Application.StartupPath + @"\Sablon Dosyaları");

                var settings = new XmlWriterSettings { Indent = true };
                var writer = XmlWriter.Create(Application.StartupPath + $@"\Sablon Dosyaları\{sablonAdi}_location.xml", settings);
                writer.WriteStartDocument();
                writer.WriteComment("BEOS  tarafından oluşturulmuştur.");
        ...
        public static void FormSablonYukle(this string sablonAdi, XtraForm frm)
        {
            ...
            try
            {
                if (File.Exists(Application.StartupPath + $@"\Sablon Dosyaları\{sablonAdi}_location.xml"))
                {
                    var reader = XmlReader.Create(Application.StartupPath + $@"\Sablon Dosyaları\{sablonAdi}_location.xml");
        ...
        public static void TabloSablonKaydet(this GridView tablo, string sablonAdi)
        {
            try
            {
                ...
                tablo.SaveLayoutToXml(Application.StartupPath + $@"\Sablon Dosyaları\{sablonAdi}.xml");
            }
            catch (Exception ex)
            {
                Messages.HataMesaji(ex.Message);
        ...
        public static void TabloSablonYukle(this GridView tablo, string sablonAdi)
        {
            try
            {
                if (File.Exists(Application.StartupPath + $@"\Sablon Dosyaları\{sablonAdi}.xml"))
                    tablo.RestoreLayoutFromXml(Application.StartupPath + $@"\Sablon Dosyaları\{sablonAdi}.xml");
            }
            catch (Exception ex)
            {
                Messages.HataMesaji(ex.Message);
        ...
        public static void TabloDisariAktar(this GridView tablo, DosyaTuru dosyaTuru, string dosyaFormati = null, string excelSayfaAdi = null)
        {
